...
The scripts load `.env.local` via dotenv, because `tsx` does not do it for you, so without that
...
The rest, meaning hero, marquee, cards, accordion and forms, is hand-built in the same idiom. Two
...
- **Alternating programme blocks** with status pills, and a single-column layout when a
  programme has no graphic, and the old layout left half the row empty.
...
1. **Team bios.** `src/lib/seed-content.ts` lists four names from public LinkedIn
   profiles with placeholder titles. The old site had no team page, so confirm before
   `/about` goes live.
